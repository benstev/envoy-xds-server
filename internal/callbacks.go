...
func (cb *Callbacks) OnStreamRequest(id int64, req *discovery.DiscoveryRequest) error {
	...
	if cb.Debug {
		log.Printf("[CB] received request for %s on stream %d: %v:%v", req.GetTypeUrl(), id, req.VersionInfo, req.ResourceNames)
	}
...
func (cb *Callbacks) OnStreamResponse(ctx context.Context, id int64, req *discovery.DiscoveryRequest, res *discovery.DiscoveryResponse) {
	...
	cb.Responses++
	if cb.Debug {
		log.Printf("[CB] responding to request for %s on stream %d", req.GetTypeUrl(), id)
	}
}
